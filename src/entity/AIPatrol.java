...
public class AIPatrol {
	...
	public void update(double deltaTime) {
		...
		double distanceFromPlayer = EngineMath.distance(entity.x + entity.getXOffset() + entity.getWidth()/2,
				entity.y + entity.getYOffset() + entity.getHeight()/2,
				cat.x + cat.getXOffset() + cat.getWidth()/2, cat.y + cat.getYOffset() + cat.getHeight()/2);
		if(distanceFromPlayer > 100) {
			if(entity.velX == 0 && entity.getEntityType() == EntityType.COW) {
	...
	private void patrolLeft(double deltaTime) {
		...
		if(entity.x < patrolEnd) {
			entity.x = patrolEnd;
			atEnd = true;
			pause(deltaTime, AnimationState.IDLE_LEFT, AnimationState.WALKING_RIGHT);
		}
		else if(entity.x > patrolStart) {
			entity.x = patrolStart;
			atEnd = true;
	...
	private boolean stopOnCollision(double deltaTime) {
	    ...
	    entity.setVelX(0);
	    if(entity.getEntityType() == EntityType.COW) {
