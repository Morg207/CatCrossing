...
public class Cow extends Entity implements Interactable {
	...
	public Cow(double x, double y, double patrolLength, Camera camera, Cat cat) {
		super(x, y, 10, 40, new Dimension(85, 52));
		this.camera = camera;
		this.cat = cat;
	...
	private void updateRestartCooldown(double deltaTime) {
	    ...
	    if (restartTimer > nextMilkTime) {
	        ...
	        restartTimer = 0;
	        milkMeter = true;
	        double distanceToPlayer = EngineMath.distance(this.x + xOffset + width/2, this.y + yOffset+height/2,
	...
	private void drawMilkProgressBar(Graphics2D g2d) {
	    ...
	    int drawX = (int)Math.round((x + 27 - camera.getXOffset()));
	    int drawY = (int)Math.round((y + 20 - camera.getYOffset()));
